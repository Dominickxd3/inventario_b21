...
export default function RegistrarBienDialog({
  ...
  const registrar = async (values: FormBien) => {
    ...
    try {
      await api.post('/inventario/bienes', {
        ...
        idResponsableActual: values.idResponsableActual ? Number(values.idResponsableActual) : null,
  ...
  const modelosDeMarca = catalogos?.modelos.filter((m) => String(m.IdMarca) === idMarca) ?? [];
  ...
  return (
    <Dialog open={abierto} onClose={alCerrar} maxWidth="sm" fullWidth>
      ...
      <DialogContent sx={{ pt: 2 }}>
        ...
        <Box component="form" id="form-registrar-bien" onSubmit={handleSubmit(registrar)} sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
          <TextField
            select
            size="small"
            label="Artículo"
